...
COMPILING THE APP
	...
	To compile the app, you may do it in one of two ways: 1. automatically with eclipse, or 2. manually through the command line.
...
RUNNING THE APP
	...
	1. Go to the bin folder, open the command line and invoke "start rmiregistry". A window with a new rmiregistry will open. This is essential for both the TestApp and Peer to work, so don't close it.

	2. To run a peer, while still inside the bin folder, on a command line window run the following command: java service.Peer <peer_ap> <MC_addr> <MC_port> <MDB_addr> <MDB_port> <MDR_addr> <MDR_port>
	...
	3. To run the TestApp, while still inside the bin folder, on a command line window, run the following command: java service.TestApp <peer_ap> <sub_protocol> <opnd_1> <opnd_2>.
